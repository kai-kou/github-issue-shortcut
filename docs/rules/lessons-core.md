...
## L-114: クラウドで gh のほぼ全操作（REST + GraphQL + search + Actions）が 403 でブロックされ MCP が唯一経路

**症状**: クラウド実行環境（`CLAUDE_CODE_REMOTE=true`）で `gh` は存在する（`gh --version` は通る）のに、
ほぼ全ての gh 操作が egress プロキシに 403 でブロックされる（2026-06-30 実機検証・Issue #121 →
2026-07-02 再検証・Issue #133 でブロック範囲が拡大）:
- repo スコープ REST（`gh api repos/{o}/{r}/...`）→「GitHub access is not enabled for this session. An org admin must connect the Claude GitHub App」
- GraphQL 全般（`gh issue/pr list`・`gh repo view`・`gh api graphql`・`--json` 付き高レベル gh）→「GraphQL proxying is not enabled」（2026-07-13 文言変化:「only the pinned set of PR-review operations is served」）
- **search 全般・非 repo REST（2026-07-02 に 403 化）**: `gh search repos/issues/code/prs`・`gh api users/{u}`・`notifications`・`user/repos` →「sessions are bound to their configured repositories」
- **Actions パス（2026-07-02 に 403 化）**: `gh variable list/set`・`gh secret list`・`gh run list`・`gh workflow list` →「Access to this GitHub Actions path is not permitted through this proxy」。GitHub Variables は MCP にも等価ツールがなく、env は Claude.ai 環境設定 / secrets-broker で供給する（`github-mcp-fallback-patterns.md` §2.4）
- `gh repo clone` も内部 API 解決で失敗。

**罠（旧フォールバックが効かない）**: `urllib.request` で `api.github.com/graphql` や `/repos/...` を
直叩きしても **同一プロキシを通るため同じ 403**。「GraphQL は urllib で代替」は誤り。

**対策**: repo スコープの GitHub 操作は **公式 GitHub MCP（`mcp__github__*`）を一次経路** にする
（`get_file_contents` / `list_issues` / `pull_request_read` / `create_pull_request` / `merge_pull_request` /
`resolve_review_thread` / `search_issues` / `actions_list` 等）。生存するのは `gh api user`・`gh api rate_limit` と
git 操作（`git clone https://...`・`git fetch/pull/push`＝git プロキシは別系統）のみ（`gh auth status` は
exit 0 でも stderr に失敗表示が出るため認証判定に使わない・2026-07-13 再検証 Issue #227）。
代替表・検証マトリクスの SSOT は `docs/rules/github-mcp-fallback-patterns.md`。
**判定基準**: `gh` が 403（GraphQL proxying / pinned set of PR-review operations / connect GitHub App / sessions are bound / Actions path not permitted）→ 即 MCP へ切替。
**保持理由**: GitHub 操作は全セッションで必須。403 で作業が完全停止するクリティカル障害のため常駐必須。

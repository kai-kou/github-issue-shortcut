...
- GraphQL 全般（`gh issue/pr list`・`gh repo view`・`gh api graphql`・`--json` 付き高レベル gh）→「GraphQL proxying is not enabled」
...
`resolve_review_thread` / `search_issues` / `actions_list` 等）。生存するのは `gh api user`・`gh api rate_limit`・
`gh auth status` と git 操作（`git clone https://...`・`git fetch/pull/push`＝git プロキシは別系統）のみ。
代替表・検証マトリクスの SSOT は `docs/rules/github-mcp-fallback-patterns.md`。
**判定基準**: `gh` が 403（GraphQL proxying / connect GitHub App / sessions are bound / Actions path not permitted）→ 即 MCP へ切替。

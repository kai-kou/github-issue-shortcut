...
> **受け渡し規約（必須・L-120）**: 調査結果は **最終メッセージ本文で受け取らない**。各エージェントに
> 絶対パス（例: `<scratchpad>/investigation-<症状ID>/<担当>.md`）への Write を指示し、最終メッセージは
> `WROTE: {パス}` の 1 行だけ返させる。起動後は **ファイルの存在と非空を確認してから Step 4 へ進む**。
> 空・一言の戻り値（`Done.` / `End.` 等）を「調査結果なし」と解釈しない（未実施を成果ゼロと扱う捏造・L-113）。
> 詳細と回収手順は `docs/rules/agent-team.md`「サブエージェント成果物の受け渡し規約」。

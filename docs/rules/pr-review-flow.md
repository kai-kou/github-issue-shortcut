...
> **背景（PR #748 の再発防止）**: 14:55 に画像パイプラインが PR を作成 → Copilot 25 分待機中にセッション切れ → 次の project-sync（11:00）まで最大 20 時間放置されるリスクがあった。hourly-routing.md の「共通プリフライト」セクションに実装詳細を記載。
...
> **アウトカムの書き方**: 「何をしたか（プロセス）」ではなく「何ができるようになったか（結果）」を1文で記述する。
> - 良い例: `「PRレビュー指摘の往復が減り、AI自動マージまでの時間が短縮される」`
> - 悪い例: `「pr-review-flow.md と CLAUDE.md を更新した」`（プロセス報告 → L-052 違反）
> - 悪い例: `「Gemini の指摘 3 件と Copilot レビューを解消してマージした」` （レビュー対応プロセスの混入 → L-102 違反・サイレント原則参照）

> **⚠️ 初回指示アウトカム報告（L-076 対策・必須）**: `--outcome` には「PR の変更内容」ではなく「ユーザーが最初に依頼した指示に対して何ができるようになったか」を書く。PR 対応（AIレビュー・マージ）フローが長引いても、最終マージ後には必ずテキストで「初回指示に対するアウトカム」を出力すること。
> ```
> ❌ "PR #N をマージしました" のみで完了報告とみなす（初回指示アウトカム欠落）
> ✅ "ユーザー指示『〇〇』に対して、△△ができるようになりました" をマージ後に出力する
> ```
> 複数 PR を含むタスクでは、全 PR マージ完了後に「タスク全体のアウトカム」を別途テキスト報告する。
>
> **チャット完了報告の構造は `completion-report-rules.md`（SSOT）に従う**: 冒頭に「ご依頼（初回指示の再掲）」、続けてアウトカム、PR 番号/URL は末尾の補足1行。マージ手順・レビュー往復を主役にしない。

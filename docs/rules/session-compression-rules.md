...
モデル別コンテキストウィンドウサイズ（現行 Opus / Sonnet = 1M トークン、Haiku = 200K トークン）と実運用上の注意は detail 版を参照。1M トークンあっても不要な情報を詰め込まない。
...
圧縮は「何をサマリーに残すか」の情報取捨選択でもある。重要な決定は Issue/PR コメントに明示コミットし、長大データ・サブエージェント全文はコンテキストに置き続けない（詳細な原則表は `docs/rules/session-compression-rules-detail.md`「圧縮プロンプトの品質設計（Context Budget Design）」を参照）。

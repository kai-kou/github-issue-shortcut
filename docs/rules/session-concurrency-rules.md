...
> **大原則 CP-4（マルチセッション共存の意識）** の実装詳細。
> **Warm 層（既定でタスク依存・E-B #20）**: Scheduled Tasks 等でマルチセッションを並行運用するプロジェクトのみ Hot 化する（`tools/check_rules_sync.sh` の `ESSENTIAL_RULES` に追記 → `--fix`）。単一セッション運用では Warm のままでよい。
...
- **TOCTOU の完全排除は不可能**: GitHub API のレイテンシにより、ラベル変更が反映されるまでに数秒のラグがある。極めて近いタイミング（数秒以内）で 2 セッションが同時起動した場合は防げない可能性がある
- **解決策**: Scheduled Tasks のスケジュールを調整し、同一パイプラインが重複する時間帯を避ける（例: 2 つのパイプラインを 2 時間以上離す）

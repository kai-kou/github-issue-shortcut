...
| 項目 | 内容 |
|------|------|
| メイン通知先 | `#all-きなこもっちーのテック深掘り` チャンネル（情報提供系） |
...
| チャンネル | 名前例 | 通知設定（推奨） |
|-----------|--------|----------------|
| メインチャンネル | `#all-きなこもっちーのテック深掘り` | **mute** 推奨（大量に流れるため） |
| 承認依頼専用チャンネル | `#approval-きなこもっちー` | **通常通知**（見落とし防止） |
| 公開・マーケティング専用チャンネル | `#publish-きなこもっちー` | **通常通知**（見落とし防止） |
...
3. **「Add App Display Name」** ダイアログが開く:
   - **Display Name（Bot Name）**: `きなこもっちーBot`（日本語 OK）
   - **Default username**: `kinakomocchi-bot`（**半角英数字・ハイフン・アンダースコアのみ**）
     > **ハマりポイント**: `きなこもっちーBot` をそのまま入力すると
     > 「Usernames can't contain special characters.」エラーになる。
     > `kinakomocchi-bot` のように ASCII のみで入力する。
4. **「Add」** ボタンをクリック
...
2. チャンネル名: `approval-きなこもっちー`（または任意）
3. プライベートチャンネル推奨（自分だけが参加）
4. 作成後、Bot を招待: `/invite @kinakomocchi-bot`

**公開・マーケティング専用チャンネル** (`SLACK_PUBLISH_CHANNEL_ID`):
1. 同様にチャンネルを作成
2. チャンネル名: `publish-きなこもっちー`（または任意）
3. プライベートチャンネル推奨
4. 作成後、Bot を招待: `/invite @kinakomocchi-bot`
...
**3 チャンネルの ID を取得する**:
- メインチャンネル（例: `#all-きなこもっちーのテック深掘り`）→ `SLACK_CHANNEL_ID`
- 承認依頼専用チャンネル（例: `#approval-きなこもっちー`）→ `SLACK_APPROVAL_CHANNEL_ID`
- 公開・マーケティング専用チャンネル（例: `#publish-きなこもっちー`）→ `SLACK_PUBLISH_CHANNEL_ID`
...
→ Default username を `kinakomocchi-bot` など **ASCII のみ** に変更する

### `channel_not_found` エラー

→ チャンネル ID が正しいか確認。チャンネルに Bot が招待されているか確認（`/invite @kinakomocchi-bot`）

...
| 2026-07-29 | **68,440 B** | +1,194 | #367: サブエージェント委譲が空回答・期待外になる要因の常駐チェック（委譲のたびに効く行動規範）。**本行で増減ログが 4 行に到達したため再棚卸しを #369 で起票済み** |
| 2026-08-01 | **69,731 B** | +1,291 | #375: 委譲結果の完了通知が後のターンに届く仕様（v2.1.211）と 3 上限。**「完了しているのに報告しない」実害の再発防止** で、委譲のたびに効く行動規範のため Hot。仕様詳細・環境変数は `agent-team.md` §F-8〜F-10（Warm）へ委譲済み |
| 2026-08-01 | **69,941 B** | +210 | 本リポジトリ固有: ベース同期でベース側 Hot から消えた「サブエージェント成果物の受け渡し規約（揮発チャネル禁止）」を Warm（`lessons/agent-teams.md` L-124・本リポジトリ実測）に残し、`lessons-core.md` の Warm 索引に 1 行だけ足した。本文の Hot 復帰（#157 の +979 B）はやめ、索引参照に置き換えて相殺している |

**現行予算は ~69.9KB / ~17,500 トークン**（ベース単体は 69,731 B）。#367 で超過したためベース側で再校正予定（ベース #369）。増減ログが 3 行を超えて増え続けるようなら、個々の追加が妥当でも累積として再棚卸しの合図とみなす（これが #146 で見落とした点）。
...
## コスト計測データの公開範囲（#189・2026-07-28 決定）

**本リポジトリは public であり、運営者個人の Claude API 利用実額をリポジトリに残さない。**

- 対象: `content/pipeline-state/cost_log.jsonl`（セッション単位の実額）・`content/analytics/cost_monthly/*.json`（月次集計）・`content/pipeline-state/daily_cost_stats.json`・**`telemetry/cost-data` ブランチの月次集計**（#202 で対象に加えた。経緯は下のサブセクション）
- **今後の追記**: `.gitignore` で停止済み（ローカルでの計測・較正は継続してよい）
- **過去分**: 2026-07-28 に `git filter-repo` で全履歴から除去し `main` へ force push した。除去したのは 70 コミット分（2026-07-17〜07-28）で、**除去後のファイル内容は 455 件すべて blob ハッシュまで一致** することを検証済み（コード・ドキュメントへの影響なし）
- **判断の根拠**: 露出していたのは月次合計 $689 規模の実額と日次内訳で、アプリの脆弱性ではないが公開する必要のない財務情報。`forks_count: 0` の間だけ履歴書き換えの副作用（fork 側の履歴分岐）が発生しないため、セルフホストを推奨し始める前のこのタイミングで実施した（#188 の議論の Phase 0）

**再発防止**: 実額をファイルに書き出す計測を追加しない。較正・分析に数値が要る場合は、丸めた指標（例: 桁数・相対比）を別ファイルに置く。トークン数（`tokens/sp` 等）は金額ではないため対象外。

### 適用範囲は「main の履歴」ではなく「public に見えるすべて」（#202・2026-07-29）

上の対応は `.gitignore` と `main` の履歴だけを見ており、**専用データブランチ `telemetry/cost-data` を見落としていた**（`tools/commit_cost_telemetry.py` が Stop hook から 1 日 1 回書き込む先・#242）。ブランチを分けても public リポジトリの一部である以上、辿れば実額が読める状態が残っていた。

- **判断**: 検討案 2（実額を落として無次元の指標だけ残す）を採用。較正の主指標は `tokens/sp` であり、**金額はドル換算の便宜値にすぎず落としても用途は損なわれない**（案 1 のブランチ削除は較正データごと失われ、別の保存先の検討が必要になる）
- **今後の書き込み**: `commit_cost_telemetry.py` の `redact_costs()` が `merge_report()` の戻り値（＝ push するペイロード）から `cost_usd` / `cost_jpy_approx` を落とす。**計算そのものは実額込みのまま** で、公開境界でだけ落とすため、ローカルの月次ファイル（gitignore 対象）を使った較正・分析は従来どおり行える
- **過去分**: 2026-07-29 に orphan コミット 1 本でブランチを作り直し force push した（飼い主の明示判断）。残したのはセッション数・トークン数の日次/月次集計とセッション単位の内訳（daily 12 日 / sessions 73 件）、落としたのは実額のみ。失われるのは日次スナップショットのコミット単位の履歴だけで、これは機械生成であり参照価値がない
- **残存リスク**: 書き換え前の旧コミットは、GitHub の GC が走るまで **SHA 直参照なら辿れる可能性がある**（#189 の `main` 書き換えと同じ性質）。免責の根拠も同じで、`forks_count: 0`（fork 側に履歴が複製されていない）かつ旧 SHA を知る第三者がいないため、実効的な露出は残らないと判断した。fork が発生した後に同種の書き換えが必要になった場合、この前提は成立しない

> **判定基準**: 「その数値は public から辿れるか？」を `main` の履歴だけでなく **全ブランチ** で問う。データブランチ・タグ・GitHub Actions のアーティファクトも公開範囲に含まれる。

> **残作業**: 書き換え前の履歴を保持する古い作業ブランチの削除は、セッション環境の git プロキシが ref 削除（`git push origin --delete`）を一律ブロックするため未完了。対象一覧と実施手順は #189 に記録している。

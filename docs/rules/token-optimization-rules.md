...
| 2026-07-26 | **68,225 B** | +979 | 本リポジトリ固有（#157）: `agent-team-summary.md` にサブエージェント成果物の受け渡し規約（揮発チャネル禁止・L-121）を復元。実測ベースの行動規範であり「削減対象外 ②」に該当するため Hot に残す |

**現行予算は ~68KB / ~17,000 トークン**（本リポジトリ実測値。ベース単体は 67,246 B）。増減ログが 3 行を超えて増え続けるようなら、個々の追加が妥当でも累積として再棚卸しの合図とみなす（これが #146 で見落とした点）。

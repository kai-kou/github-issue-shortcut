...
| 起票成功率（送信 → GitHub 反映） | 99% 以上（失敗時は入力内容を失わない） |
...
| FR-4 | 初回オンボーディングで GitHub App のインストール（対象リポジトリの選択）へ誘導できること | MUST | M1 |
...
| FR-24 | 同一内容の二重送信を防止できること（送信中の再タップ抑止 + 直近送信内容との照合。GitHub API に冪等性キーがないため） | MUST | M1 |
...
6. App インストール誘導: 起票先リポジトリが「App インストール済み ∩ ユーザーがアクセス可」に含まれない場合、GitHub App のインストールページへ誘導する UX を提供すること（MUST）。
...
3. 送信中は送信ボタンを無効化し（二重タップ防止）、サーバー側でも直近送信内容（リポジトリ + タイトル + 本文のハッシュ・短時間ウィンドウ）と照合して重複起票を防ぐこと（MUST・FR-24）。
...
| ID | 分類 | 要件 | レベル |
|----|------|------|--------|
| NFR-1 | パフォーマンス | 再訪時（app shell precache 済み）の起動から入力可能まで 1 秒未満（サブ秒）であること。中位 Android 実機 + 4G 相当で計測 | MUST |
| NFR-2 | パフォーマンス | ショートカット起動から Issue 作成完了まで 10 秒以内（タイトルのみ 5 秒以内）・3 タップ以内で完了できること | MUST |
| NFR-3 | パフォーマンス | 初期バンドルを常に監視し、起票フローに不要なコードを遅延ロードすること（起動速度 > リッチ UI の優先順位） | SHOULD |
...
| OQ-7 | サービス名・ドメイン（workers.dev サブドメインかカスタムドメインか）。`__Host-` Cookie と TWA（Digital Asset Links）はドメイン確定が前提 | M1 公開前 |
...
- [01-lean-canvas.md](./01-lean-canvas.md) — リーンキャンバス（作成予定）
- [02-ux-flow.md](./02-ux-flow.md) — UX フロー詳細（作成予定）
- [03-architecture.md](./03-architecture.md) — アーキテクチャ詳細設計（作成予定）
- [04-milestones.md](./04-milestones.md) — マイルストーン計画（作成予定）
- リサーチ（2026-07-10 実施）

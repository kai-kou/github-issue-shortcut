...
| NFR-14 | コスト | Cloudflare 無料枠（Workers リクエスト数・Rate Limiting binding・static assets 配信無料）内で運用できる設計であること（P3 で永続層は不使用）。枠超過が近づいたら検知できること。**検知手段（#171）**: Cloudflare には「リクエスト数が無料枠の X% で通知」に相当するアラート種別が存在しない（Budget / Billing Usage Alert は金額ベースで Free プランでは機能しない）ため、`docs/routines/sprint-session.md` Step 1.6 で GraphQL Analytics API から当日の requests / errors を毎スプリント実測し、70% / 90% の閾値で対処する | MUST |
...
| 再送はネットワーク失敗のみが対象（4xx/5xx 対象外）・キューは 24h で失効。さらに再送はアプリを開いている間しか走らない（SW の Background Sync は未発火のまま撤去・#177） | オフラインキュー（FR-22）は API エラーと分離。24h 超の未送信は自動再送を打ち切り、失敗扱い（`queue_expired`）で D2-1 の一覧に残して手動で再送・破棄させる（SHOULD・#91 で実装）。下書きへ戻す案は採らない（既に起票済みの可能性を利用者へ明示せずに再入力させると重複起票を誘発するため） |
...
| PR-2 | プライバシーポリシーに、収集データ（GitHub トークン・アカウント情報とその保存先が **利用者の端末** であること・二重起票防止の一時記録）・保持期間・削除方法・**サーバー基盤に残る記録（Workers Logs のサンプリング率と保持期間）**・**問い合わせ窓口**・**越境処理（Cloudflare, Inc. の所在国と、処理が日本国外でも行われうること＝「外的環境の把握」・#190 で追加）** を明記すること。サーバー側で個人データを保持しないこと自体をポリシーに明示する | MUST | M1 |
...
| 無料枠超過 | 低 | NFR-14。P3 で永続層（D1 書込）そのものが無くなり、残るのは Workers リクエスト数のみ。Cloudflare 側にリクエスト数の閾値アラートが無いため、スプリントルーティン Step 1.6 で GraphQL Analytics API から実測する（#171）。実測値は直近ピークで 308 requests/日 = 枠の 0.3%（2026-07-28 時点） |
...
| # | 事項 | 期限の目安 |
|---|------|------|
| OQ-1 | ~~クエリパラメータ付き「ホーム画面に追加」ショートカットのタップ時挙動（standalone WebAPK かブラウザタブか）の実機検証~~ **決定済み: クエリパラメータ付き URL を単体でホーム画面に追加しても初期選択は反映されない**（WebAPK は start_url で起動するという Android の仕様）。プリセット付き起動はアプリ内の「保存済みショートカット」一覧、または PWA のアイコン長押しメニュー（manifest shortcuts・最大 3 個）から行う。UI 側は `shortcuts.placementGuideNote` で利用者に明示済み | 決定済み |

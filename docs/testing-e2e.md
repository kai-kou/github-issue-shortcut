...
- [ ] 機内モードで起票 → キュー表示 → **アプリを完全に閉じた状態** から復帰し、再度アプリを開いた
      ときに自動再送が走る（再送はページ側経路のみ。SW の Background Sync は #177 で撤去したため、
      アプリを開かない限り送信されないことも確認する。**重複の有無は端末内の冪等性キー
      （`src/issues/sentRequestIds.ts` の 26 時間窓）+ `e2e/offline-queue.spec.ts` の
...
> **キーボード追従・WebAPK コールドスタートからのタイトルのみ起票・機内モードからの自動再送
> （GitHub 反映 1 件・重複なし。担い手は SW ではなくアプリ再起動時のページ側経路だったことが
> 後日判明・#177）・実指タップ・配色の視認性のいずれも問題なし** と報告された。
...
- 実行: `npm run test:measure`（`npm run e2e -- kpi.spec.ts` と同義。結果を `test-results/kpi-metrics.json` に書き出す）。
- 計測する指標（すべて機械的に取得可能）:
  ...
  - **タップ数（プリフィル起動のみ）**: 起動（ホーム画面ショートカットのタップ・1 回として計上）から起票完了までのページ内タップ数を実測し、`#124` の Done Criteria（3 タップ以内）をハードアサートする。
- シナリオ: ① ショートカット起動（`/new?repo=...` でリポジトリ初期選択済み・タイトルは入力）② プリフィル起動（`/new?repo=...&title=...` で repo + title 両方指定済み・送信タップのみで完了・3 タップ以内をハードアサート）③ 通常起動（リポジトリ選択タップ込み）。

...
| §1.2 実物の製品 UI | 起票フォームの実スクリーンショット（`npm run screenshots` で元画像 → `npm run lp:assets` で切り出し。生成物はコミット対象） |
...
| §2 モバイル縦積み順 | 縦積み順と見出し 2 行は 390px 幅の実描画で目視確認。横スクロールなしは 360px / 1280px で機械検証（`e2e/lp.spec.ts`） |
| §2 CTA 2 系統 | ヘッダー右上（小）+ ヒーロー直下（主） |
| §3 ダークモード | `prefers-color-scheme` でトークンを切替。アプリ本体（`src/App.css`）と同系統の配色（accent・primary・text・border は同値、背景と余白は LP 用に拡張） |
...
- **ビルドなし・依存ゼロ**（HTML + CSS + 依存ゼロの JS 約 120 行）。フレームワーク・CSS ライブラリを入れない
...
- `axe-core`（wcag2a / wcag2aa / wcag22aa）で違反ゼロを機械検証（`e2e/lp.spec.ts`）。日本語 / ライト / モバイルと English / ダーク / デスクトップの両方を監査する

 ...
 *
 * サーバー側の予約と同じく **claim（予約）→ 送信 → 失敗時は release（解放）** の順で使う。
...
/** 送信済み（または送信中）の内容 1 件分の記録。内容そのものではなくキーだけを持つ。 */
...
/**
 * 内容の同一性キー。JSON 配列にしてからシリアライズし、フィールド境界の曖昧さ
 * （例: repo="a", title="b\nc" と repo="a\nb", title="c" が同じキーになる）を避ける
 * （サーバー実装のハッシュ対象と同じ組み立て方）。端末内に留まるためハッシュ化はしない。
 */
export function submissionKey(input: SubmissionInput): string {
  return JSON.stringify([input.repo, input.title, input.body, input.labels]);
}

/** 照合ウィンドウを過ぎた記録を落とす（判定に使われないうえ、放置すると際限なく増えるため）。 */
export function pruneSubmissions(records: SubmissionRecord[], now: number): SubmissionRecord[] {
  return records.filter((r) => now - r.at < DUPLICATE_SUBMISSION_WINDOW_MS);
}
 ...
 */
export function claimSubmission(input: SubmissionInput, now: number = Date.now()): boolean {
  const next = claimSubmissionRecord(load(), submissionKey(input), now);
...
export function releaseSubmission(input: SubmissionInput): void {
  persist(releaseSubmissionRecord(load(), submissionKey(input)));
}

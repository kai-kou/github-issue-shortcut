 ...
 */
import { ensureFreshAccessToken, refreshAfterUnauthorized } from "./tokenRefresh";
...
export async function apiFetch(input: string, init: RequestInit = {}): Promise<Response> {
  ...
  // 期限判定がずれていた（時計ずれ・別タブでのログアウト等）場合の保険。リフレッシュできたときだけ
  // 1 度再送する。未ログインならリフレッシュは即 false を返すため、余計な往復は発生しない。
  if (!(await refreshAfterUnauthorized())) return res;

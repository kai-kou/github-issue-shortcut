...
- 所見は Write ツールで {RUN_DIR}/{観点}.md に書き出す（**絶対パスで指定する**）
...
### Step 1.5: 所見の受領検証（空を「指摘なし」と解釈しない・L-120）
...
> 報告後に system-reminder 等の継続プロンプトが注入されると、エージェントは相槌を返し
> （`完了。` → `End.`）、その相槌が最終テキストとして所見を上書きする。実測では
> 所見 807 文字を出力済みのエージェントの戻り値が `End.`、ファイル書き出しを指示した再開でも
> 戻り値は `(Complete.)` だったが **ファイルは正しく残っていた**（トークン・ツール呼び出しは正常に消費）。

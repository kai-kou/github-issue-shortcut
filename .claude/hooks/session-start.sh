...
if [ -n "${GH_TOKEN:-}" ]; then
  echo "gh CLI: authenticated ($(gh api user --jq '.login' 2>/dev/null || echo 'token present'))" >&2
else
...
if [ "$_skip_cleanup" = true ]; then
  echo "[cleanup] SKIP（headless or 明示スキップ）" >&2
  ...
  # main（デフォルトブランチ）の最新化（checkout はしない）。
...
import json,sys
try:
    prs=json.load(sys.stdin); rtm=[p for p in prs if p.get('status')=='ready_to_merge']
    print(json.dumps(rtm))
except Exception: print('[]')
" 2>/dev/null || echo "[]")
  _rtm_count=$(echo "$_rtm" | python3 -c "import json,sys; print(len(json.load(sys.stdin)))" 2>/dev/null || echo "0")
  if [ "$_rtm_count" -gt 0 ]; then
    printf '\n🔔 即時マージ可能な PR が %s 件あります。問題なし判定コメント → squash マージを最優先で処理してください:\n' "$_rtm_count"
    echo "$_rtm" | python3 -c "
import json,sys
for pr in json.load(sys.stdin):
    print(f\"  PR #{pr.get('pr_number','?')}: {pr.get('title','')[:60]} ({pr.get('elapsed_min',0)}分経過)\")
" 2>/dev/null || true
    printf '\n'
  fi
